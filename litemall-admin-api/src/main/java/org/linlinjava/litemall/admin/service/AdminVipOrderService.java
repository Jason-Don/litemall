...
public class AdminVipOrderService {
    private final Log logger = LogFactory.getLog(AdminVipOrderService.class);
    ...
    public Object list(Integer userId, String orderSn, List<Short> orderStatusArray, String payType,
        ...
        List<LitemallVipOrder> orderList = vipOrderService.querySelective(userId, orderSn, orderStatusArray, payType, page, limit, sort, order);
        return ResponseUtil.okList(orderList);
    }

    public Object detail(Integer id) {
        LitemallVipOrder vipOrder = vipOrderService.findById(id);
        Map<String, Object> orderVo = new HashMap<String, Object>();

//        List<LitemallOrderGoods> orderGoods = orderGoodsService.queryByOid(id);
        UserVo user = userService.findUserVoById(vipOrder.getUserId());
        Map<String, Object> data = new HashMap<>();
        data.put("order", vipOrder);
...
//
    ...
    public Object pay(String body) {
        ...
        LitemallVipOrder vipOrder = vipOrderService.findById(orderId);
        if (vipOrder == null) {
            return ResponseUtil.badArgument();
        }

        // 如果订单是已取消状态，则不能确认付款
        if (vipOrder.getOrderStatus().equals(OrderUtil.STATUS_CANCEL)
                || vipOrder.getOrderStatus().equals(OrderUtil.STATUS_AUTO_CANCEL)) {
        ...
        updateVipOrder.setOrderStatus(OrderUtil.STATUS_PAY);
        updateVipOrder.setActualPrice(new BigDecimal(actualPrice));
        updateVipOrder.setPayTime(LocalDateTime.now());
        if (vipOrderService.update(updateVipOrder) == 0) {
            return ResponseUtil.updatedDateExpired();
        }

        //关联用户表，并更新余额
        LitemallUser userDo = userService.findById(vipOrder.getUserId());
        LitemallUser updateUser = new LitemallUser();
        updateUser.setId(userDo.getId());
        updateUser.setVipOrderId(vipOrder.getId());
        updateUser.setBalance(userDo.getBalance().add(updateVipOrder.getActualPrice()));
